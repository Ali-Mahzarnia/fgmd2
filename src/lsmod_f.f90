! --------------------------------------------------------------------------
! fGMD.f90: A moidfied version of gglasso.f90
! --------------------------------------------------------------------------
! 
! USAGE:
! 
...
!   alpha=mixing paramtger
!   gamu= the set of eignevalues coputed for lambdader=0
!   GGder= Graham of second derivative
!   lambdader= regularization par of second der. penalty
!
! OUTPUT:
! 
...
!   The features !added to the original package: mixing parameter alpha, curvature penalizing for functional regression and 
!   cross-validations, optimized Fortran core function to speed up the curvature penalization updates, 
!   progress reports and time estimations. This package does not work independently from the MFSGrp package and it 
!   does not interfere with the functions of gglasso package due to slight name differences.
! Author of the original gglasso :
...
! --------------------------------------------------
SUBROUTINE lsmod_f (bn,bs,ix,iy,gam,nobs,nvars,x,y,pf,dfmax,pmax,nlam,flmin,ulam,&
