...
#' @method coef fGMD
coef.fGMD <- function(object, s = NULL, ...) {
...
#' @export
predict.fGMD <- function(object, newx, s = NULL, type = c("class", 
    "link"), ...) {
    type <- match.arg(type)
    loss <- class(object)[[2]]
    b0 <- t(as.matrix(object$b0))
...
#' @export
print.fGMD <- function(x, digits = max(3, getOption("digits") - 
    3), ...) {
    cat("\nCall: ", deparse(x$call), "\n\n")
...
#' @export
coef.cv.fGMD <- function(object, s = c("lambda.1se", "lambda.min"), 
    ...) {
    if (is.numeric(s)) 
    ...
    coef(object$fGMD.fit, s = lambda, ...)
}

#' @export
predict.cv.fGMD <- function(object, newx, s = c("lambda.1se", 
    "lambda.min"), ...) {
    if (is.numeric(s)) 
        lambda <- s else if (is.character(s)) {
        s <- match.arg(s)
    ...
    predict(object$fGMD.fit, newx, s = lambda, ...)
} 
